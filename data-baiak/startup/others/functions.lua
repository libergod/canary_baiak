		...
		for i = 1, #value.itemPos do
			...
			if tile then
				...
				if item and value.itemId ~= false then
					item:setAttribute(ItemAttribute_t::ACTIONID, index)
				end
				if value.itemId == false and tile:getTopDownItem() then
					tile:getTopDownItem():setAttribute(ItemAttribute_t::ACTIONID, index)
				end
				if value.itemId == false and tile:getTopTopItem() then
					tile:getTopTopItem():setAttribute(ItemAttribute_t::ACTIONID, index)
				end
				if value.itemId == false and tile:getGround() then
					tile:getGround():setAttribute(ItemAttribute_t::ACTIONID, index)
				end
			end
		end
	end
...
function loadLuaMapUnique(tablename)
	...
	for index, value in pairs(tablename) do
		...
		if tile then
			...
			if item then
				item:setAttribute(ItemAttribute_t::UNIQUEID, index)
			end
		end
	end
end
...
function loadLuaMapSign(tablename)
	...
	for index, value in pairs(tablename) do
		...
		if tile then
			...
			if item then
				item:setAttribute(ItemAttribute_t::TEXT, value.text)
			end
		end
	end
end
