...
local function doCheckArea()
	local upConer = {x = 1139, y = 652, z = 7}       -- upLeftCorner
	local downConer = {x = 1162, y = 676, z = 7}     -- downRightCorner
...
function heartDestructionQuake.onUse(player, item, fromPosition, itemEx, toPosition)
	...
	local config = {
		...
		newPos = {x = 1150, y = 672, z = 7},
	}

	local pushPos = {x = 1124, y = 660, z = 7}
		...
		if item.itemid == 8911 then
			if player:getPosition().x == pushPos.x and player:getPosition().y == pushPos.y and player:getPosition().z == pushPos.z then
				...
				if doCheckArea() == false then
					...
					Game.createMonster("Foreshock", {x = 1150, y = 664, z = 7}, false, true)
					...
					local vortex = Tile({x = 1141, y = 664, z = 7}):getItemById(23482)
					if vortex then
