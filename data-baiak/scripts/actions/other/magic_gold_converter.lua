local data = {
	...
	coins = {
		...
		[ITEM_CRYSTAL_COIN] = ITEM_GOLD_INGOT
		}
}
			...
			if item:isContainer() then
				...
				for fromid, toid in pairs(data.coins) do
					if item:getId() == fromid and item:getCount() == 100 then						
						...
						conv:setAttribute(ItemAttribute_t::CHARGES, conv:getAttribute(ItemAttribute_t::CHARGES) - 1)
...
local function start_converter(pid, itemid)
	...
	local item = player:getItemById(itemid,true)
		if player:getItemCount(itemid) >= 1 then
			if item:hasAttribute(ItemAttribute_t::CHARGES) then
				local charges_n = item:getAttribute(ItemAttribute_t::CHARGES)
				if charges_n >= 1 then
