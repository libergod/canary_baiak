...
local cerebellumHeal = CreatureEvent("cerebellumHeal")
function cerebellumHeal.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType)
	if attacker and attacker:isPlayer() then
		if creature:getName():lower() == "cerebellum" then
			...
			if (primaryType == COMBAT_ENERGYDAMAGE or secondaryType == COMBAT_ENERGYDAMAGE and not creature:getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT, 88888)) then
				...
				addEvent(function(cid)
					...
					creature:say("Cerebellum heals him self!", TALKTYPE_ORANGE_2)
					return true
