...
function snowBall.onSay(player, words, param)
	...
	if param == SNOWBALL.comandoAtirar then
		...
		if player:getStorageValue(10107) >= os.time() then
			player:sendCancelMessage(SNOWBALL.prefixo .. SNOWBALL.mensagemExausted)
			return true
		end
		...
		player:setStorageValue(10107, os.time() + SNOWBALL.muniExhaust)
