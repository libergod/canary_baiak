                    ...
                    if tonumber(owner) == player:getGuid() then
                        ...
                        for _, targetPlayer in ipairs(gMembers) do
                            if targetPlayer then                               
								...
								targetPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You received from Guild Bank %d transferable coins.", tournamentsCoins))
								db.query(string.format("INSERT INTO `store_history`(`account_id`, `mode`, `description`, `coin_type`, `coin_amount`, `time`) VALUES (%s, %s, %s, %s, %s, %s)", targetPlayer:getAccountId(), "0", db.escapeString("[Guild Bank] - Online Members TCs"), "2", "100", os.time()))
                            end
                        end
                    end
                end
