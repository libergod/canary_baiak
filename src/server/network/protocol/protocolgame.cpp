...
void ProtocolGame::parseBestiarysendMonsterData(NetworkMessage &msg) {
	...
	if (currentLevel > 1) {
		...
		newmsg.addDouble(0);// Version 13.10 (mitigation)
	}
...
void ProtocolGame::sendCreatureIcon(const Creature* creature) {
	...
	if (icon != CREATUREICON_NONE) {
		...
		msg.add<uint16_t>(0);
	} else if (useHazard && creaturePlayer && creaturePlayer->getHazardSystemReference() > 0 && creaturePlayer->getHazardSystemPoints() > 0) {
