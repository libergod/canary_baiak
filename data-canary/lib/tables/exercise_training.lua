...
function ExerciseEvent(playerId, tilePosition, weaponId, dummyId)
	...
	if not weapon:isItem() or not weapon:hasAttribute(ItemAttribute_t::CHARGES) then
		player:sendTextMessage(MESSAGE_FAILURE, "The selected item is not a training weapon, the training has stopped.")
		LeaveTraining(playerId)
		return false
	end

	local weaponCharges = weapon:getAttribute(ItemAttribute_t::CHARGES)
	...
	weapon:setAttribute(ItemAttribute_t::CHARGES, (weaponCharges - 1))
	...
	if weapon:getAttribute(ItemAttribute_t::CHARGES) <= 0 then
		weapon:remove(1)
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Your training weapon has disappeared.")
		LeaveTraining(playerId)
		return false
